...
- Uploaded all necessary files to Github (for Joyce as well)

## Summary

This project aims to examine whether lower-income neighborhoods in Chicago have reduced access to grocery stores compared to higher-income areas. By asking “How do income levels of individuals in Chicago impact the number of grocery stores present in the area?”, we hoped to see a relationship between median household income within neighborhoods and the number of grocery stores people have access to. This topic is important to us because disparities in food access can increase food insecurity and even create disparities in health. We believe our analysis can further contribute to urban planning, health equity, and increased food access in a city where we plan to work. To further our investigation, we obtained a dataset of all Chicago groceries and the Chicago census data containing median household income levels. The Grocery Store Dataset included important information about grocery stores in Chicago such as the name of the store, address, and their operation status. The census data contained important information about the demographics of people living in Chicago. While this data was collected a while ago, we believe that census data can not be updated as it was collected in a point in time. This was considered when picking the dataset and during analysis. It ultimately carried important information on median incomes in certain zip codes throughout Chicago. After we obtained the two datasets, we cleaned the datasets. We mainly standardized the datasets to ensure consistency and reorganized the datasets for easy interpretation. Once the cleaning process was completed, the two datasets were combined based on zip codes because it was a key attribute that both data sets possessed. We then counted the number of grocery stores under each zip code to create data visualizations that helped us examine patterns and trends within the merged dataset. Through our analysis, we found that there was no strong correlation between income levels in certain neighborhoods and the number of grocery stores people living in those areas have access to. This was due to many external factors and considerations with the data. Grocery store distribution may be influenced by a combination of multiple additional factors, such as population density, neighborhood size, transportation, and urban planning strategies. The project also revealed that food accessibility cannot solely be measured through the number of grocery stores. However, we were able to gain valuable insight into the overall grocery landscape in Chicago and income distribution. The project re-emphasized the complexity of food inequalities within the greater Chicago area, and encourages deeper exploration of this important topic.
